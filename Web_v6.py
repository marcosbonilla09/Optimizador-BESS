...
def simular_bess_milp(HW):
    """
    EMS diario LP (GLOP) que MINIMIZA coste de compra de red.

    Variables (por QH):
      - g_load_kWh: energía de red usada directamente para cubrir carga
      - pv_load_kWh: FV usada directamente para cubrir carga
      - c_grid_kWh: energía de red para cargar batería
      - c_pv_kWh: excedente FV para cargar batería
      - d_kWh: descarga batería para cubrir carga (no exporta batería)
      - e_kWh: energía almacenada (SOC en kWh)
      - vertido_kWh: FV no usada (exportada)

    Balance carga:
      load = g_load + pv_load + d

    Balance FV:
      pv = pv_load + c_pv + vertido

    SOC:
      e[t+1] = e[t] + ETA_C*(c_grid + c_pv) - (1/ETA_D)*d

    Objetivo (por día):
      min sum_t price[t] * (g_load[t] + c_grid[t])
    """
    ...
    # ---- helpers: coger arrays como en tu app (globals o session_state) ----
    def _get(name, default=None):
        if name in globals():
            return globals()[name]
        return st.session_state.get(name, default)

    base_dt_index        = _get("base_dt_index")
    load_vec             = np.asarray(_get("load_vec"), dtype=float)
    precio_vec           = np.asarray(_get("precio_vec"), dtype=float)  # en tu DF suele ser eur/MWh
    precio_venta_fv_vec  = np.asarray(_get("precio_venta_fv_vec", np.zeros_like(precio_vec)), dtype=float)
    pot_contratada       = np.asarray(_get("pot_contratada", np.full_like(precio_vec, 1e9)), dtype=float)
    generacion_vec       = np.asarray(_get("generacion_vec", np.zeros_like(precio_vec)), dtype=float)
    excedentes_vec       = np.asarray(_get("excedentes_vec", np.zeros_like(precio_vec)), dtype=float)
    ...
    # ---- Conversión precio: si viene en €/MWh lo pasamos a €/kWh ----
    # (si ya lo tienes en €/kWh, pon PRICE_IS_EUR_MWH = False)
    PRICE_IS_EUR_MWH = True
    if PRICE_IS_EUR_MWH:
        price_kwh = precio_vec / 1000.0
        sell_kwh  = precio_venta_fv_vec / 1000.0
    else:
        price_kwh = precio_vec.copy()
        sell_kwh  = precio_venta_fv_vec.copy()

    # ---- Parámetros batería ----
    E_CAP   = float(HW["E_CAP"])                 # kWh
    SOC_MIN = float(HW.get("SOC_MIN", 0.10))     # pu
    SOC_MAX = float(HW.get("SOC_MAX", 0.90))     # pu
    E_MIN   = E_CAP * SOC_MIN
    E_MAX   = E_CAP * SOC_MAX
    ...
    E_MAX_QH = P_LIM_KW * 0.25                         # kWh/QH

    # ---- Estado inicial ----
    # Si tienes SOC inicial en HW lo usas; si no, arrancas a SOC_MIN (simple y conservador)
    SOC0 = float(HW.get("SOC0", SOC_MIN))
    e_prev = np.clip(SOC0 * E_CAP, E_MIN, E_MAX)
    ...
    # ---- Índices por día (asumiendo QH: 96 pasos) ----
    ...
    for d, idxs in by_day.items():
        ...
        # Construye solver diario
        solver = pywraplp.Solver.CreateSolver("GLOP")
        inf = solver.infinity()

        # Variables diarias (indexadas 0..n-1)
        g_load = [solver.NumVar(0.0, inf, f"g_load_{k}") for k in range(n)]
        pv_load = [solver.NumVar(0.0, inf, f"pv_load_{k}") for k in range(n)]
        c_grid = [solver.NumVar(0.0, E_MAX_QH, f"c_grid_{k}") for k in range(n)]
        c_pv   = [solver.NumVar(0.0, E_MAX_QH, f"c_pv_{k}") for k in range(n)]
        dch    = [solver.NumVar(0.0, E_MAX_QH, f"d_{k}") for k in range(n)]
        e      = [solver.NumVar(E_MIN, E_MAX, f"e_{k}") for k in range(n+1)]
        vert   = [solver.NumVar(0.0, inf, f"vert_{k}") for k in range(n)]

        # SOC inicial del día
        solver.Add(e[0] == float(e_prev))

        # Restricciones por QH del día
        for k, t in enumerate(idxs):
            load_t = float(load_vec[t])
            pv_t   = max(0.0, float(generacion_vec[t]))

            # Balance de carga: load = g_load + pv_load + d
            ct = solver.Constraint(load_t, load_t)
            ct.SetCoefficient(g_load[k], 1.0)
            ct.SetCoefficient(pv_load[k], 1.0)
            ct.SetCoefficient(dch[k], 1.0)

            # Balance FV: pv = pv_load + c_pv + vert
            ct2 = solver.Constraint(pv_t, pv_t)
            ct2.SetCoefficient(pv_load[k], 1.0)
            ct2.SetCoefficient(c_pv[k], 1.0)
            ct2.SetCoefficient(vert[k], 1.0)

            # SOC: e[k+1] = e[k] + ETA_C*(c_grid + c_pv) - (1/ETA_D)*d
            ct3 = solver.Constraint(0.0, 0.0)
            ct3.SetCoefficient(e[k+1], 1.0)
            ct3.SetCoefficient(e[k], -1.0)
            ct3.SetCoefficient(c_grid[k], -ETA_C)
            ct3.SetCoefficient(c_pv[k],   -ETA_C)
            ct3.SetCoefficient(dch[k],    1.0/ETA_D)

            # (Opcional) si quieres impedir simultaneidad “física” sin MILP:
            # limita suma de energías en el QH (relajación simple)
            solver.Add(c_grid[k] + c_pv[k] + dch[k] <= E_MAX_QH)

        # Cierre diario SOC (para que actúe "diario" de verdad)
        solver.Add(e[n] == e[0])

        # Objetivo: minimizar coste compra red = price*(g_load + c_grid)
        obj = solver.Objective()
        for k, t in enumerate(idxs):
            obj.SetCoefficient(g_load[k], float(price_kwh[t]))
            obj.SetCoefficient(c_grid[k], float(price_kwh[t]))
        obj.SetMinimization()

        status = solver.Solve()
        if status != pywraplp.Solver.OPTIMAL:
            raise RuntimeError(f"No se encontró óptimo en día {d}.")

        # Volcar resultados a arrays globales
        for k, t in enumerate(idxs):
            g = g_load[k].solution_value()
            pvld = pv_load[k].solution_value()
            cg = c_grid[k].solution_value()
            cpv = c_pv[k].solution_value()
            dd = dch[k].solution_value()
            ee = e[k+1].solution_value()
            vv = vert[k].solution_value()

            g_load_kWh[t] = g
            autoconsumo_kWh[t] = pvld
            carga_red_kWh[t] = cg
            carga_exc_kWh[t] = cpv
            descarga_kWh[t] = dd
            energia_kWh[t] = ee
            ...
            cons_red_pro_kWh[t] = g + cg
            p_grid_kW[t] = 4.0 * (g + cg)

        # SOC para el siguiente día
        e_prev = float(e[n].solution_value())

    # ingresos por vertido FV (si lo quieres reportar)
    ...
    out = pd.DataFrame({
        ...
        "excedentes_kWh": excedentes_vec,  # referencia (sin BESS)
        "precio_eur_mwh": precio_vec,      # dejo tu unidad original
