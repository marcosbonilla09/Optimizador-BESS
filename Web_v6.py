...
def simular_bess_milp(HW):
    """
    Heurística diaria (sin solver) que:
      1) FV -> autoconsumo, excedente -> carga batería, resto -> vertido
      2) Descarga en QH más caros (solo para cubrir load, sin exportar por batería)
      3) Si la FV no llena la batería, carga de red en QH más baratos,
         respetando hueco hasta potencia contratada y límites de inversor/batería.
      4) Sin simultaneidad: nunca carga y descarga en el mismo QH.
    Devuelve exactamente las mismas columnas que tu salida actual.
    """
    import numpy as np
    import pandas as pd

    # ---------- Entradas globales (las prepara _prepare_base_signals_from_session_state) ----------
    # (misma lógica que tu función actual)
    def _get(name, default):
        return globals().get(name, default)

    base_dt_index        = _get("base_dt_index", None)
    load_vec             = np.asarray(_get("load_vec", None), dtype=float)
    precio_vec           = np.asarray(_get("precio_vec", None), dtype=float)                 # €/MWh
    pot_contratada       = np.asarray(_get("pot_contratada", np.full_like(precio_vec, 1e9)), dtype=float)  # kW
    generacion_vec       = np.asarray(_get("generacion_vec", np.zeros_like(precio_vec)), dtype=float)      # kWh/QH
    excedentes_vec       = np.asarray(_get("excedentes_vec", np.zeros_like(precio_vec)), dtype=float)      # ref sin BESS
    precio_venta_fv_vec  = np.asarray(_get("precio_venta_fv_vec", np.zeros_like(precio_vec)), dtype=float) # €/MWh

    if base_dt_index is None:
        base_dt_index = pd.date_range("2000-01-01", periods=len(load_vec), freq="15min")
    dt = pd.to_datetime(pd.Index(base_dt_index))
    ...
    # ---------- Precios a €/kWh ----------
    # En tu código actual PRICE_IS_EUR_MWH=True, lo mantengo.
    price_kwh = precio_vec / 1000.0
    sell_kwh  = precio_venta_fv_vec / 1000.0

    # ---------- Parámetros batería ----------
    E_CAP   = float(HW["E_CAP"])                   # kWh
    SOC_MIN = float(HW.get("SOC_MIN", 0.10))
    SOC_MAX = float(HW.get("SOC_MAX", 0.90))
    E_MIN   = E_CAP * SOC_MIN
    E_MAX   = E_CAP * SOC_MAX

    ETA_C   = float(HW.get("ETA_C", 0.95))
    ETA_D   = float(HW.get("ETA_D", 0.95))

    P_LIM_KW = float(min(HW["P_BATT"], HW["P_INV"]))  # kW
    E_MAX_QH = P_LIM_KW * 0.25                        # kWh/QH (energía AC por cuarto de hora)

    # Estado inicial
    SOC0 = float(HW.get("SOC0", SOC_MIN))
    e_prev = float(np.clip(SOC0 * E_CAP, E_MIN, E_MAX))

    # ---------- Salidas (mismos nombres que tu DF final) ----------
    carga_red_kWh     = np.zeros(T)
    carga_exc_kWh     = np.zeros(T)
    descarga_kWh      = np.zeros(T)
    energia_kWh       = np.zeros(T)
    vertido_kWh       = np.zeros(T)
    autoconsumo_kWh   = np.zeros(T)
    g_load_kWh        = np.zeros(T)
    cons_red_pro_kWh  = np.zeros(T)
    p_grid_kW         = np.zeros(T)

    # ---------- Agrupar índices por día ----------
    day_key = dt.normalize()
    by_day = {}
    for i, d in enumerate(day_key):
        by_day.setdefault(d, []).append(i)

    # ---------- Helper: pre-simulación FV (para decidir si hace falta carga de red) ----------
    def _simulate_pv_only(idxs, e0):
        e = float(e0)
        for t in idxs:
            load = max(0.0, float(load_vec[t]))
            pv   = max(0.0, float(generacion_vec[t]))

            pv_to_load = min(load, pv)
            pv_surplus = pv - pv_to_load

            # carga con excedente FV (AC) limitada por potencia y por capacidad disponible
            cap_room_ac = max(0.0, (E_MAX - e) / max(1e-12, ETA_C))
            cpv = min(pv_surplus, E_MAX_QH, cap_room_ac)

            # update SOC
            e = min(E_MAX, e + ETA_C * cpv)
    ...
    for d, idxs in by_day.items():
        ...
        # 1) Estimar si la FV del día ya llenaría la batería (sin red)
        e_end_pv_only = _simulate_pv_only(idxs, e_prev)
        need_grid_ac_total = max(0.0, (E_MAX - e_end_pv_only) / max(1e-12, ETA_C))

        # 2) Elegir ventanas caras/baratas del día (ranking por precio)
        prices_day = price_kwh[idxs]
        order_cheap = idxs[np.argsort(prices_day)]          # más baratos primero
        order_exp   = idxs[np.argsort(-prices_day)]         # más caros primero

        # Nº máximo de QH “útiles” para mover toda la energía utilizable del día
        e_usable = max(0.0, E_MAX - E_MIN)
        k_move = int(np.ceil(e_usable / max(1e-12, E_MAX_QH)))
        k_move = int(np.clip(k_move, 1, len(idxs)))

        cheap_set = set(order_cheap[:k_move].tolist()) if need_grid_ac_total > 1e-9 else set()
        exp_set   = set(order_exp[:k_move].tolist())

        # 3) Simulación cronológica del día (respetando SOC)
        e = float(e_prev)
        grid_need_left = float(need_grid_ac_total)

        for t in idxs:
            load = max(0.0, float(load_vec[t]))
            pv   = max(0.0, float(generacion_vec[t]))

            # --- FV a carga (autoconsumo primero) ---
            pv_to_load = min(load, pv)
            pv_surplus = pv - pv_to_load

            # Queda por cubrir tras FV (antes de batería)
            rem_load = load - pv_to_load

            # --- Decidir descarga (si QH caro) ---
            dch = 0.0
            if (t in exp_set) and (rem_load > 1e-12) and (e > E_MIN + 1e-12):
                # energía AC máxima descargable por potencia y SOC
                max_dch_ac_by_soc = max(0.0, (e - E_MIN) * ETA_D)  # si descargo y, e baja y/ETA_D
                dch = min(rem_load, E_MAX_QH, max_dch_ac_by_soc)

            # Regla NO simultaneidad:
            # si descargo, NO cargo ni con PV excedente ni con red en este QH
            cpv = 0.0
            cgrid = 0.0

            if dch <= 1e-12:
                # --- Carga con excedente FV (si no estoy descargando) ---
                if pv_surplus > 1e-12 and e < E_MAX - 1e-12:
                    cap_room_ac = max(0.0, (E_MAX - e) / max(1e-12, ETA_C))
                    cpv = min(pv_surplus, E_MAX_QH, cap_room_ac)

                # --- Carga de red (si hace falta llenar y QH barato) ---
                if (grid_need_left > 1e-12) and (t in cheap_set) and (e < E_MAX - 1e-12):
                    # potencia de red ya usada para load (kW)
                    p_load_kW = 4.0 * rem_load  # rem_load aún no descontó batería (porque dch=0)
                    # hueco hasta potencia contratada
                    p_room_kW = max(0.0, float(pot_contratada[t]) - p_load_kW)
                    # límite energético por potencia contratada en este QH
                    e_room_grid_ac = 0.25 * p_room_kW

                    cap_room_ac = max(0.0, (E_MAX - e) / max(1e-12, ETA_C))
                    cgrid = min(E_MAX_QH, e_room_grid_ac, cap_room_ac, grid_need_left)

            # --- Vertido: excedente FV que no se autoconsume ni se carga ---
            vert = max(0.0, pv_surplus - cpv) if dch <= 1e-12 else max(0.0, pv_surplus)

            # --- Actualizar energía almacenada ---
            # e_next = e + ETA_C*(cgrid+cpv) - (1/ETA_D)*dch
            e = e + ETA_C * (cgrid + cpv) - (dch / max(1e-12, ETA_D))
            e = float(np.clip(e, E_MIN, E_MAX))

            # --- Consumo red para load (g_load) ---
            # g_load = load - pv_to_load - dch  (sin exportar batería)
            g = max(0.0, rem_load - dch)

            # --- Guardar arrays ---
            autoconsumo_kWh[t]  = pv_to_load
            carga_exc_kWh[t]    = cpv
            carga_red_kWh[t]    = cgrid
            descarga_kWh[t]     = dch
            vertido_kWh[t]      = vert
            energia_kWh[t]      = e
            g_load_kWh[t]       = g

            cons_red_pro_kWh[t] = g + cgrid
            p_grid_kW[t]        = 4.0 * (g + cgrid)

            # --- Reducir necesidad de carga de red pendiente ---
            if cgrid > 0:
                grid_need_left = max(0.0, grid_need_left - cgrid)

        # Fin de día: arrastro SOC al siguiente día (tu requisito es “llenar al máximo”,
        # y la heurística ya intenta cargar lo máximo posible con FV+red barata)
        e_prev = float(e)

    # ingresos por vertido FV
    ingreso_vertido_eur = vertido_kWh * sell_kwh

    out = pd.DataFrame({
        "datetime": pd.Index(base_dt_index).values,
        "load_kWh": load_vec,
        "cons_red_pro_kWh": cons_red_pro_kWh,
        "excedentes_kWh": excedentes_vec,
        ...
        "soc_pu": energia_kWh / max(1e-9, E_CAP),
        "maximetro_kW": p_grid_kW,
